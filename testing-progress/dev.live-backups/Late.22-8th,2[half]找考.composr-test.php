...
function/*placement:analy-main*/不同_qu($sq,$or){}
function 找考/*analy.2-assemblr*/($贴协_s,$贴协_f){$贴协=array_merge($贴协_s,$贴协_f);/*echo "<br>Start time: ".microtime()."<br>";*/foreach($贴协 as $sq=>$类对){
	 ...
	 if($类对->财产["专名"][0]=="P"){$类对->并行歪找($类对->财产["地点"],$lim="P");}
else if($类对->财产["专名"][0]=="K"){$类对->并行歪找($类对->财产["地点"],$lim="K");$类对->并行查找($类对->财产["地点"],$lim="K");}
else if($类对->财产["专名"][0]=="B"){$类对->并行歪找($类对->财产["地点"]);}
else if($类对->财产["专名"][0]=="R"){$类对->并行查找($类对->财产["地点"]);}
else if($类对->财产["专名"][0]=="Q"){$类对->并行歪找($类对->财产["地点"]);$类对->并行查找($类对->财产["地点"]);}
else if($类对->财产["专名"][0]=="N"){$类对->N找($类对->财产["地点"]);}}
...
<b>Output</b> | Output-cheq
<table>
<?php while($总和_s||$总和_f){?>
<tr><td><?php if($_SESSION["set"]=="w"){if($总和_f!==array()){echo 不同_qu($总和_f[0],"f");}}else{if($总和_s!==array()){echo 不同_qu($总和_s[0],"s");}}?></span>></td>
<td><?php if($_SESSION["set"]=="w"){if($总和_s!==array()){echo 不同_qu($总和_s[0],"f");}}else{if($总和_f!==array()){echo 不同_qu($总和_f[0],"s");}}?></span>></td></tr>
<?php array_shift($总和_f);array_shift($总和_s);}} 
...
[Late-8th]to prod out-pu: sort, by side, total E_ accor-sq "ranked_E_tot_s/f" $计数=max(count($贴协_s),count($贴协_f));
	by the count of the highest side, echo when isset the lower side
