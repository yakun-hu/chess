...
function/*innate-M*/级斗($一,$二){if($一-$二>0){return"prot-E";}elseif($一-$二<0){return"eE";}else{return"neutr";}}
...
<tr><?php if($_SESSION["set"]=="w"){if(/*front-row*/$总和_f!==array()){echo /*incl-prov for empt-td*/不同_qu($总和_f[0],"直",$贴协);}else{echo"<td>ff</td>";}if($总和_s!==array()){echo 不同_qu($总和_s[0],"反",$贴协);}}else{if($总和_s!==array()){echo /*incl-prov for empt-td*/不同_qu($总和_s[0],"直",$贴协);}else{echo"<td></td>";}if(/*right-row*/$总和_f!==array()){echo 不同_qu($总和_f[0],"反",$贴协);}}?>
</tr><?php array_shift($总和_f);array_shift($总和_s);}}?></table>

<?php /*calls*/找考($贴协_s,$贴协_f);/*test*/echo"<br>";foreach(array_merge($贴协_s,$贴协_f) as $sq=>$类对){/*echo$sq." | ";print_r($类对->E_sum);echo$类对->财产["专名"]."<br>";*/}?>
<!-- key issue 1) the val in $总和 is not the sq; 2) $_SESSION["set"] cont "b"<s>or"w"<f> 
...
<!-- test-prog 
理论: if one of the arrs-empt, then a <td>missn; when and only when # the left misn, the right will bump<wron>theref, onl-when the off-orien,side is short; so, the left-most cond nee'+1,nest
...
3qk3/2pn1p2/Q7/3N2b1/3PB1b1/2N1P3/Pr4P1/R4K2 b - - 0 21 <with-N>
	...
	Undefined array key "b1"; b1 is empt in b-p; it's not foun in the B-co | b1 is not in the E_出s in lin-14 (E, $W) rel: Attempt to read property "E_出" on null when does 14-rea?
		...
		/*can't-mrg,Q*/ : if Q, run-both; elseif, runs 1
r7/4bpkp/6p1/qp1QP3/p6P/6P1/5P2/RR4K1 b - - 0 38 looks-gud
